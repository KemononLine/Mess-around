...
void moveSnake() //  移动小蛇及相关处理函数
{
	...
	//  根据用户按键，设定新蛇头的位置
	if ((moveDirection == 'w' && curDirection != 's')||(curDirection=='w'&&moveDirection=='s')) // 向上移动
	{
		newHead_i = oldHead_i - 1;
		curDirection = 'w';
	}
	else if ((moveDirection == 's' && curDirection != 'w')||(curDirection=='s'&&moveDirection=='w')) // 向下移动
	{
		newHead_i = oldHead_i + 1;
		curDirection = 's';
	}
	else if ((moveDirection == 'a' && curDirection != 'd')||(curDirection=='a'&&moveDirection=='d')) // 向左移动
	{
		newHead_j = oldHead_j - 1;
		curDirection = 'a';
	}
	else if ((moveDirection == 'd' && curDirection != 'a')||(curDirection == 'd' && moveDirection == 'a')) // 向右移动
	{
		newHead_j = oldHead_j + 1;
		curDirection = 'd';
	}
...
void startup()  //  初始化函数
{
	...
	curDirection = moveDirection;	//维护住小蛇现在的移动方向
